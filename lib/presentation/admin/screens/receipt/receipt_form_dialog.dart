...
class _ReceiptFormDialogState extends State<ReceiptFormDialog> {
  ...
  List<Map<String, dynamic>> _selectedProducts = []; // [{productId, productName, quantityOrdered, receiptDetailId?}]
    ...
    if (widget.editMode) {
      ...
      provider.getSelectedReceiptDetail().then((r) {
        ...
        setState(() {
          ...
          _storeId = r.storeId;
          _selectedProducts = r.details.map((d) => {
          ...
          for (var product in _selectedProducts) {
            ...
            if (widget.isReceiveMode) {
              _receivedQuantityControllers[product['productId']] = TextEditingController(
  ...
  void _addProduct(Product product) {
    ...
    setState(() {
      ...
      if (widget.isReceiveMode) {
        _receivedQuantityControllers[product.id] = TextEditingController(text: '0');
      }
  ...
  Future<void> _handleSubmit() async {
    ...
    if (widget.isReceiveMode) {
      ...
      final details = _selectedProducts.map((p) => {
        'receiptDetailId': p['receiptDetailId'],
        'receivedQuantity': int.parse(_receivedQuantityControllers[p['productId']]!.text),
      }).toList();
      ...
      final details = _selectedProducts.map((p) => {
        ...
        'quantityOrdered': int.parse(_quantityControllers[p['productId']]!.text),
      }).toList();
      final id = provider.selectedReceiptId;
      if (id != null) {
    ...
    if (success) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(widget.isReceiveMode ? 'Nhận hàng thất bại' : widget.editMode ? 'Cập nhật thất bại' : 'Tạo mới thất bại')),
      );
    }
  }
                      ...
                      child: Column(
                        ...
                        children: [
                            ...
                            children: [
                              Expanded(
                                child: DropdownButtonFormField<int?>(
                                  ...
                                  decoration: _decoration('Nhà cung cấp', required: true),
                                  items: [
                                    const DropdownMenuItem<int?>(value: null, child: Text('Chọn nhà cung cấp')),
                                    ...supplierProvider.suppliers.map((s) => DropdownMenuItem<int?>(value: s.id, child: Text(s.name))),
                                  ],
                                  onChanged: widget.isReceiveMode ? null : (v) => setState(() => _supplierId = v),
                                ),
                              ),
                              const SizedBox(width: 14),
                              Expanded(
                                child: DropdownButtonFormField<int?>(
                                  value: _storeId != null && storeProvider.stores.any((s) => s.id == _storeId) ? _storeId : null,
                                  ...
                                  onChanged: widget.isReceiveMode ? null : (v) => setState(() => _storeId = v),
                                ),
                              ),
                            ],
                          ),
                          ...
                          if (_selectedProducts.isNotEmpty) ...[
                            Text(
                              widget.isReceiveMode ? 'Sản phẩm nhận hàng' : 'Sản phẩm đã thêm',
                                      ...
                                      children: [
                                        ...
                                        if (widget.isReceiveMode)
                                          SizedBox(
                                            width: 120,
                                            child: TextFormField(
                                              controller: _receivedQuantityControllers[productId],
                                              keyboardType: TextInputType.number,
                                              decoration: InputDecoration(
                                                labelText: 'Nhận',
                                                isDense: true,
                                              ...
                                              validator: (value) {
                                                ...
                                                final qty = int.tryParse(value);
                                                if (qty == null || qty < 0) return '>= 0';
                                                if (qty > product['quantityOrdered']) return '<= ${product['quantityOrdered']}';
                                                return null;
                                              },
                                            ),
                                          ),
                            ...
                            children: [
                              ...
                              ElevatedButton(
                                ...
                                style: ElevatedButton.styleFrom(
                                  backgroundColor: widget.isReceiveMode
